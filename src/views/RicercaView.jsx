...
export default function RicercaView({ dipendenti, assegnazioni, macchine }) {
    const [searchType, setSearchType] = useState("persona");
    ...
    const results = useMemo(() => {
        ...
        if (searchType === "persona") {
            ...
            if (!dip) return [];
            return assegnazioni
                .filter((a) => a.dipendenteId === dip.id)
                .map((a) => ({
                    data: a.data,
                    turno: a.turno,
                    macchina: macchine.find((m) => m.id === a.macchinaId)?.nome || a.macchinaId,
        ...
        if (searchType === "macchina") {
            ...
            if (!mac) return [];
            return assegnazioni
                .filter((a) => a.macchinaId === mac.id)
                .map((a) => {
                    const d = dipendenti.find((dd) => dd.id === a.dipendenteId);
                    return {
                        data: a.data,
                        turno: a.turno,
                        macchina: mac.nome,
        ...
        return [];
    }, [searchType, searchVal, dipendenti, assegnazioni]);
        ...
        <div className="fade-in">
            ...
            {results.length > 0 ? (
                ...
                <div className="card" style={{ textAlign: "center", padding: 40 }}>
                    ...
                    <p style={{ color: "var(--text-muted)", fontSize: 13 }}>Nessun risultato trovato per "{searchVal}"</p>
                </div>
            ) : (
                <div className="card" style={{ textAlign: "center", padding: 40 }}>
