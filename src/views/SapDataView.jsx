                        ...
                        <div style={{ display: "flex", borderRadius: 8, border: "1px solid var(--border-light)", overflow: "hidden", boxShadow: "0 1px 2px rgba(0,0,0,0.05)" }}>
                            {[{ value: "ALL", label: "Tutti" }, ...["A","B","C","D"].map(t => ({ value: t, label: t }))].map(({ value, label }, idx, arr) => (
                                <button key={value} onClick={() => { setPage(0); setSelectedTurno(value); }}
                                    style={{ padding: "8px 12px", fontSize: 14, fontWeight: 600, border: "none", borderRight: idx < arr.length - 1 ? "1px solid var(--border-light)" : "none", cursor: "pointer", backgroundColor: selectedTurno === value ? "var(--accent)" : "var(--bg-secondary)", color: selectedTurno === value ? "white" : "var(--text-muted)", boxShadow: selectedTurno === value ? "0 1px 3px rgba(0,0,0,0.15)" : "none", transition: "all 0.15s" }}>
                                    {label}
                                </button>
                            ))}
                        </div>
